...
- **D-62. Lazy metadata fetch.** Only pay a Linux `statx` (or a Windows extra
  ...
  *Realized by `sys::EnumPlan { want_stat, want_file_id, want_reparse_file_id }`,
  computed once by the engine from the fetch mask (`want_stat` = the mask intersects
  the size/time/attr fields) and the follow/cycle settings (`want_reparse_file_id` =
  cycle detection on **and** `FollowLinks::Always`, needed only to loop-guard a
  followed reparse point, so identity is fetched for symlinks alone; `want_file_id`
